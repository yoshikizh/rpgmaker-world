...
Scene_Title.prototype.commandWindowRect = function() {
  ...
  const wy = Graphics.boxHeight / 2 - 96 + offsetY;
...
Scene_Map.prototype.update = function() {
  ...
  if ($gameMap.isBattleMap() && this._sceneBattle){
    this._sceneBattle.update()
  }
  if ($gameMap.isWorldMap()){
    this.updateWorldMap()
  }
...
Scene_Map.prototype.onMapTouch = function() {
  ...
  // this._spriteset._touchEffectSprite.play(TouchInput.x,TouchInput.y)
  // if (!SceneManager._scene.isStartBattle()){
...
Scene_Map.prototype.createHomeButtons = function() {
  if (ConfigManager.touchUI) {
    // this.createMenuButton();
    this.createHeroMenuButton();
    this.createPackageMenuButton();
    this.createMissionMenuButton();
    this.createSummonMenuButton();
  }
};

Scene_Map.prototype.createPlayerInfo = function(){
  this._spritePlayerInfo = new SpritePlayerInfo()
  this._spritePlayerInfo.x = 32
  this._spritePlayerInfo.y = 16
  this.addWindow(this._spritePlayerInfo)
}
...
Scene_Map.prototype.createHeroMenuButton = function() {
  const rect = new Rectangle(Config.width - 160, 32, 128, 64)
  this._menuHero = new WindowBaseClickable(rect)
  this._menuHero.drawText("英雄", 0, 0, 112, "center")
  this._menuHero.setclickHandle(() => {
    console.log("click hero")
  })
  this.addWindow(this._menuHero);
}

Scene_Map.prototype.createPackageMenuButton = function() {
  const rect = new Rectangle(Config.width - 160, 32 + 80, 128, 64)
  this._menuPackage = new WindowBaseClickable(rect)
  this._menuPackage.drawText("背包", 0, 0, 112, "center")
  this._menuPackage.setclickHandle(() => {
    console.log("click package")
  })
  this.addWindow(this._menuPackage);
}

Scene_Map.prototype.createMissionMenuButton = function() {
  const rect = new Rectangle(Config.width - 160, 32 + 80 * 2, 128, 64)
  this._menuMission = new WindowBaseClickable(rect)
  this._menuMission.drawText("任务", 0, 0, 112, "center")
  this._menuMission.setclickHandle(() => {
    console.log("click mission")
  })
  this.addWindow(this._menuMission);
}

Scene_Map.prototype.createSummonMenuButton = function() {
  const rect = new Rectangle(Config.width - 160, 32 + 80 * 3, 128, 64)
  this._menuSummon = new WindowBaseClickable(rect)
  this._menuSummon.drawText("召唤", 0, 0, 112, "center")
  this._menuSummon.setclickHandle(() => {
    console.log("click summon")
  })
  this.addWindow(this._menuSummon);
}
